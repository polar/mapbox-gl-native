...
ParseResult parseCompoundExpression(const std::string name, const mbgl::style::conversion::Value& value, ParsingContext ctx) {
    ...
    for (std::size_t j = 0; j < definition.size(); j++) {
        ...
        if (
            ...
            signature->params.get<std::vector<type::Type>>().size() == length
        ) {
